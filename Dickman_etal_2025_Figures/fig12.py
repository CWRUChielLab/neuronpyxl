...
def plot_ax(bmp, ax, sigma, delta, ylabel=True, contour=True,cv=False):
    ...
    ax.scatter([params["unloaded"][xcol]], [params["unloaded"][ycol]], marker="*", c="magenta",\
            edgecolors="white", s=400, linewidths=1.5,alpha=1,zorder=2,label="Unloaded")
    ax.scatter([params["loaded"][xcol]], [params["loaded"][ycol]], marker="*", c="springgreen",\
               edgecolors="white", s=400, linewidths=1.5,alpha=1,zorder=2,label="Loaded")
    return mesh
...
error = "cv"
"""
pc12 = plot_ax(f"{error}1", axs[0], 1.5, 0.1,True,False,False if error == "std" else True)
pc22 = plot_ax(f"{error}2", axs[1], 2.5, 0.1, False,False,False if error == "std" else True)
"""
axs[0].set_xlabel(r"$\bar{g}$ of slow potassium in B64s ($\mu$S)",fontsize=16)
axs[0].set_ylabel(r"$\bar{g}$ of B30 to B63 connection ($\mu$S)",fontsize=16)
axs[1].set_xlabel(r"$\bar{g}$ of slow potassium in B64s ($\mu$S)",fontsize=16)
...
location = "bottom"
shrink = 1
fig.colorbar(pc11, ax=axs[0], shrink=shrink, location=location,\
               pad=0.05,orientation=orientation,label="Phase duration (s)")
fig.colorbar(pc21, ax=axs[1], shrink=shrink, location=location,\
        pad=0.05,orientation=orientation,label="Phase duration (s)")
