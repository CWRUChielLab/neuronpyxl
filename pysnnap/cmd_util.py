...
parser.add_argument('--vonly', action='store_true', help="If --vonly is entered, will only record membrane potentials of each cell and the time. If boyh --syn and --vonly are passed, will default to --vonly.")
...
def run_sim(name: str, file: str, step:float=-1., duration:float=10000., method:int=2, atol:float=1e-5, interp:float=-1, syn:bool=False, vonly:bool=False, noise:tuple=None, teq:float=1000.0):
    """Runs a single NEURON simulation from the spreadsheet specified in the file parameter.

    Args:
        name (str): name of the simulation to run. This is without the .smu suffix in the spreadsheet.
        file (str): file path to the excel spreadsheet which defines the network model.
        step (float, optional): _description_. Defaults to -1.0.
        duration (float, optional): _description_. Defaults to 10000.0.
        ...
        interp (float, optional): if provided, interpolates all data to the provided timestep. Defaults to -1.0
        syn (bool, optional): if provided, records all synaptic currents. Defaults to False.
        vonly (bool, optional): if provided, only records membrane potentials. Defaults to False.
    """
    ...
    # Save metadata
    nb.generate_metadata(voltage_only=vonly)
...
if __name__ == "__main__":
    ...
    if func == "gen_mods":
        ...
        run_sim(name=args.name, file=args.file, duration=args.duration, step=args.step, method=args.method, atol=args.atol, interp=args.interp, syn=args.syn, vonly=args.vonly, noise=args.noise, teq=args.teq)